...
int main(int argc, char **argv) {
  if (argc != 2) {
    ...
    std::cerr << "Available kernels: copy, naive, coalesced, coalesced_no_bank_conflicts" << std::endl;
    return 1;
  }
  ...
  } else if (kernel_name == "coalesced_no_bank_conflicts") {  
  ...
  std::cout << kernel_name << " kernel executed." << std::endl;
